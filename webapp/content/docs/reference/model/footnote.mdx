...
```#footnote.entry(note, separator:, clearance:, gap:, indent:) -> content```

<TypeTable
  type={{
    'note': {
      'description': 'The footnote for this entry. Its location can be used to determine the footnote counter state.  ```typst #show footnote.entry: it => {   let loc = it.note.location()   counter(footnote).display(at: loc, "1: ")   it.note.body }  Customized #footnote[Hello] listing #footnote[World! 🌏] ```',
      'type': 'content',
      'default': 'None'
    },
    'separator': {
