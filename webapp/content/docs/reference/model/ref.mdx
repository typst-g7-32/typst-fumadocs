...
```#ref(target, supplement:, form:) -> content```
...
<TypeTable
  type={{
    ...
    'supplement': {
      'description': 'A supplement for the reference.  If the [`form{:typst}`](/docs/reference/model/ref/#parameters-form) is set to `"normal"{:typst}`:  - For references to headings or figures, this is added before the referenced number. - For citations, this can be used to add a page number.  If the [`form{:typst}`](/docs/reference/model/ref/#parameters-form) is set to `"page"{:typst}`, then this is added before the page number of the label referenced.  If a function is specified, it is passed the referenced element and should return content.  ```typst #set heading(numbering: "1.") #show ref.where(   form: "normal" ): set ref(supplement: it => {   if it.func() == heading {     "Chapter"   } else {     "Thing"   } })  = Introduction <intro> In @intro, we see how to turn Sections into Chapters. And in @intro[Part], it is done manually. ```',
