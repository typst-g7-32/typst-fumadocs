...
```#gradient.linear(stops, space:, relative:, dir, angle) -> gradient```
...
```#gradient.radial(stops, space:, relative:, center:, radius:, focal-center:, focal-radius:) -> gradient```
...
```#gradient.conic(stops, angle:, space:, relative:, center:) -> gradient```
...
```#gradient.sharp(steps, smoothness:) -> gradient```
...
```#gradient.repeat(repetitions, mirror:) -> gradient```
...
```#gradient.focal-center() -> none array```
...
```#gradient.focal-radius() -> none ratio```
