---
title: "PNG"
description: "Documentation for functions related to PNG in Typst."
---
...
PNGs can have transparent backgrounds. By default, Typst will output a PNG with an opaque white background. You can make the background transparent using `#set page(fill: none){:typst}`. Learn more on the [`page{:typst}` function's reference page](/docs/reference/layout/page/#parameters-fill).

## Exporting as PNG

### Command Line

Pass `--format png{:typst}` to the `compile{:typst}` or `watch{:typst}` subcommand or provide an output file name that ends with `.png{:typst}`.
...
- Which resolution to render at by specifying `--ppi{:typst}` followed by a number of pixels per inch. The default is `144{:typst}`.
...
- The resolution at which the pages should be rendered, as a number of pixels per inch. The default is `144{:typst}`.
