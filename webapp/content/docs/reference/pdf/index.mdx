---
title: "PDF"
description: "Documentation for functions related to PDF in Typst."
---
...
PDF is Typst's default export format. Running the `compile{:typst}` or `watch{:typst}` subcommand without specifying a format will create a PDF. When exporting to PDF, you have the following configuration options:

- Which [PDF standards](#pdf-standards) Typst should enforce conformance with by specifying `--pdf-standard{:typst}` followed by one or multiple comma-separated standards. Valid standards are `1.4{:typst}`, `1.5{:typst}`, `1.6{:typst}`, `1.7{:typst}`, `2.0{:typst}`, `a-1b{:typst}`, `a-1a{:typst}`, `a-2b{:typst}`, `a-2u{:typst}`, `a-2a{:typst}`, `a-3b{:typst}`, `a-3u{:typst}`, `a-3a{:typst}`, `a-4{:typst}`, `a-4f{:typst}`, `a-4e{:typst}`, and `ua-1{:typst}`. By default, Typst outputs PDF-1.7-compliant files.
- You can disable PDF tagging completely with `--no-pdf-tags{:typst}`. By default, Typst will always write _Tagged PDF_ to provide a baseline level of accessibility. Using this flag, you can turn tags off. This will make your file inaccessible and prevent conformance with accessible conformance levels of PDF/A and all parts of PDF/UA.
...
- Which PDF standards Typst should enforce conformance with. By default, Typst outputs PDF-1.7-compliant files. You can choose the PDF version freely between 1.4 and 2.0. Valid additional standards are `A-1b{:typst}`, `A-1a{:typst}`, `A-2b{:typst}`, `A-2u{:typst}`, `A-2a{:typst}`, `A-3b{:typst}`. `A-3u{:typst}`, `A-3a{:typst}`, `A-4{:typst}`, `A-4f{:typst}`, `A-4e{:typst}`, and `UA-1{:typst}`.
...
- Wrap all decorative elements without a semantic meaning in [`pdf.artifact{:typst}`](/docs/reference/pdf/artifact/).
...
- **If you decided on part one, two, or three,** you should typically choose the accessible conformance level of your part, indicated by a lowercase `a{:typst}` at the end. If your document is inherently inaccessible, e.g. an artist's portfolio that cannot be boiled down to alternative descriptions, choose conformance level `u{:typst}` instead. Only if this results in a compiler error, e.g. because you used code points from the Unicode Private Use Area, use the basic level `b{:typst}`.
...
Typst exposes PDF-specific functionality in the global `pdf{:typst}` module. See below for the definitions it contains.

This module contains some functions without a final API. They are designed to enhance accessibility for documents with complex tables. This includes [`table-summary{:typst}`](/docs/reference/pdf/table-summary/), [`header-cell{:typst}`](/docs/reference/pdf/header-cell/), and [`data-cell{:typst}`](/docs/reference/pdf/data-cell/). All of these functions will be removed in a future Typst release, either through integration into table functions or through full removal. You can enable these functions by passing `--features a11y-extras{:typst}` or setting the `TYPST_FEATURES{:typst}` environment variable to `a11y-extras{:typst}`. In the web app, these features are not available at this time.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td><code><a href="/reference/pdf/artifact/">artifact</a></code></td>
      <td>Marks content as a PDF artifact.</td>
    </tr>
    <tr>
      <td width="20px" align="center">—</td>
      <td><code><a href="/reference/pdf/attach/">attach</a></code></td>
      <td>A file that will be attached to the output PDF.</td>
    </tr>
    <tr>
      <td width="20px" align="center">—</td>
      <td><code><a href="/reference/pdf/data-cell/">data-cell</a></code></td>
      <td>Explicitly defines this cell as a data cell.</td>
    </tr>
    <tr>
      <td width="20px" align="center">—</td>
      <td><code><a href="/reference/pdf/header-cell/">header-cell</a></code></td>
      <td>Explicitly defines a cell as a header cell.</td>
    </tr>
    <tr>
      <td width="20px" align="center">—</td>
      <td><code><a href="/reference/pdf/table-summary/">table-summary</a></code></td>
      <td>A summary of the purpose and structure of a complex table.</td>
    </tr>
  </tbody>
</table>
