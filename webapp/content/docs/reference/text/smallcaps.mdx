...
```#smallcaps(all:, body) -> content```

<TypeTable
  type={{
    'all': {
      'description': 'Whether to turn uppercase letters into small capitals as well.  Unless overridden by a show rule, this enables the `c2sc{:typst}` OpenType feature.  ```typst #smallcaps(all: true)[UNICEF] is an agency of #smallcaps(all: true)[UN]. ```',
      'type': 'bool',
