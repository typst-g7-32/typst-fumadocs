...
```#raw(text, block:, lang:, align:, syntaxes:, theme:, tab-size:) -> content```
...
<TypeTable
  type={{
    ...
    'lang': {
      'description': 'The language to syntax-highlight in.  Apart from typical language tags known from Markdown, this supports the `"typ"{:typst}`, `"typc"{:typst}`, and `"typm"{:typst}` tags for [Typst markup](/docs/reference/syntax/#markup), [Typst code](/docs/reference/syntax/#code), and [Typst math](/docs/reference/syntax/#math), respectively.  ```typst ```typ This is *Typst!* ```  This is ```typ also *Typst*```, but inline! ```',
      'type': 'none | str',
...
```#raw.line(number, count, text, body) -> content```
