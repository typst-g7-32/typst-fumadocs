---
title: "Plugin"
description: "Documentation for the `plugin` function."
---
...
Typst WebAssembly plugins need to follow a specific [protocol](/docs/reference/foundations/plugin/#protocol). To run as a plugin, a program needs to be compiled to a 32-bit shared WebAssembly library. Plugin functions may accept multiple [byte buffers](/docs/reference/foundations/bytes/) as arguments and return a single byte buffer. They should typically be wrapped in idiomatic Typst functions that perform the necessary conversions between native Typst types and bytes by leveraging [`str{:typst}`](/docs/reference/foundations/str/#constructor), [`bytes{:typst}`](/docs/reference/foundations/bytes/#constructor), and [data loading functions](/docs/reference/data-loading/).
...
<TypstPreview
  ...
  #concat("hello", "world")
`}
  image='/typst-fumadocs/assets/563eb9b72603c710f73876546a243de1.png'
  alt='Preview'
  editable={true}
/>
...
- Take `n{:typst}` 32-bit integer arguments `a_1{:typst}`, `a_2{:typst}`, ..., `a_n{:typst}` (interpreted as lengths, so `usize/size_t{:typst}` may be preferable), and return one 32-bit integer.
- The function should first allocate a buffer `buf{:typst}` of length `a_1 + a_2 + ... + a_n{:typst}`, and then call `wasm_minimal_protocol_write_args_to_buffer(buf.ptr){:typst}`.
- The `a_1{:typst}` first bytes of the buffer now constitute the first argument, the `a_2{:typst}` next bytes the second argument, and so on.
- The function can now do its job with the arguments and produce an output buffer. Before returning, it should call `wasm_minimal_protocol_send_result_to_host{:typst}` to send its result back to the host.
- To signal success, the function should return `0{:typst}`.
- To signal an error, the function should return `1{:typst}`. The written buffer is then interpreted as an UTF-8 encoded error message.
...
- `(import "typst_env" "wasm_minimal_protocol_write_args_to_buffer" (func (param i32))){:typst}` Writes the arguments for the current function into a plugin-allocated buffer. When a plugin function is called, it [receives the lengths](#exports) of its input buffers as arguments. It should then allocate a buffer whose capacity is at least the sum of these lengths. It should then call this function with a `ptr{:typst}` to the buffer to fill it with the arguments, one after another.
- `(import "typst_env" "wasm_minimal_protocol_send_result_to_host" (func (param i32 i32))){:typst}` Sends the output of the current function to the host (Typst). The first parameter shall be a pointer to a buffer (`ptr{:typst}`), while the second is the length of that buffer (`len{:typst}`). The memory pointed at by `ptr{:typst}` can be freed immediately after this function returns. If the message should be interpreted as an error message, it should be encoded as UTF-8.
...
In the example below, we load the plugin `hello-mut.wasm{:typst}` which exports two functions: The `get(){:typst}` function retrieves a global array as a string. The `add(value){:typst}` function adds a value to the global array.

We call `add{:typst}` via the transition API. The call `mutated.get(){:typst}` on the derived module will observe the addition. Meanwhile the original module remains untouched as demonstrated by the `base.get(){:typst}` call.
