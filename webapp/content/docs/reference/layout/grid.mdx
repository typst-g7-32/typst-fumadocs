...
```#grid(columns:, rows:, gutter:, column-gutter:, row-gutter:, inset:, align:, fill:, stroke:, children) -> content```
...
```#grid.cell(body, x:, y:, colspan:, rowspan:, inset:, align:, fill:, stroke:, breakable:) -> content```
...
```#grid.hline(y:, start:, end:, stroke:, position:) -> content```
...
```#grid.vline(x:, start:, end:, stroke:, position:) -> content```
...
```#grid.header(repeat:, level:, children) -> content```
...
```#grid.footer(repeat:, children) -> content```
