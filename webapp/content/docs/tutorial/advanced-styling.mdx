...
<img src="/typst-fumadocs/assets/3-advanced-team-settings" alt="The team settings" />
...
<TypstPreview
  ...
  #lorem(600)
`}
  image='/typst-fumadocs/assets/39e9b50304410f17939935dca5419e30.png'
  alt='Preview'
  editable={true}
/>

You are already familiar with most of what is going on here. We set the text size to `11pt{:typst}` and the font to Libertinus Serif. We also enable paragraph justification and set the page size to US letter.

The `header{:typst}` argument is new: With it, we can provide content to fill the top margin of every page. In the header, we specify our paper's title as requested by the conference style guide. We use the `align{:typst}` function to align the text to the right.

Last but not least is the `numbering{:typst}` argument. Here, we can provide a [numbering pattern](/docs/reference/model/numbering/) that defines how to number the pages. By setting it to `"1"{:typst}`, Typst only displays the bare page number. Setting it to `"(1/1)"{:typst}` would have displayed the current page and total number of pages surrounded by parentheses. And we could even have provided a completely custom function here to format things to our liking.

## Creating a title and abstract

Now, let's add a title and an abstract. We'll start with the title. Typst comes with a [`title{:typst}`](/docs/reference/model/title/) function. Let's start by providing our title as an argument:
...
- One with the selector `title{:typst}` and the rule `set text(size: 17pt){:typst}`
- One with the selector `title{:typst}` and the rule `set align(center){:typst}`
...
`}
  image='/typst-fumadocs/assets/c2969040db7c93ade24a461b5ba45124.png'
  alt='Preview'
  editable={true}
/>
...
`}
  image='/typst-fumadocs/assets/4790f48caf3dcd84b8fea8c0326a2aee.png'
  alt='Preview'
  editable={true}
/>

The two author blocks are laid out next to each other. We use the [`grid{:typst}`](/docs/reference/layout/grid/) function to create this layout. With a grid, we can control exactly how large each column is and which content goes into which cell. The `columns{:typst}` argument takes an array of [relative lengths](/docs/reference/layout/relative/) or [fractions](/docs/reference/layout/fraction/). In this case, we passed it two equal fractional sizes, telling it to split the available space into two equal columns. We then passed two content arguments to the grid function. The first with our own details, and the second with our supervisors'. We again use the `align{:typst}` function to center the content within the column. The grid takes an arbitrary number of content arguments specifying the cells. Rows are added automatically, but they can also be manually sized with the `rows{:typst}` argument.

Looking at the authors and the title, they are a bit too close together. You can address this by using another show-set rule to configure the space below the title. The title, the grid, paragraphs, and all other elements that Typst arranges from the top to the bottom of the page are called _blocks._ Each block is controlled by the [`block{:typst}`](/docs/reference/layout/block/) function. It controls behaviors like their distance and whether a block can contain a page break. That means that we can write another show-set rule that selects the title to set the block spacing:
...
`}
  image='/typst-fumadocs/assets/41f337ec4affe1f096129a26c54f7f2e.png'
  alt='Preview'
  editable={true}
/>

Well done! One notable thing is that we used a set rule within the content argument of `align{:typst}` to turn off justification for the abstract. This does not affect the remainder of the document even though it was specified after the first set rule because content blocks _scope_ styling. Anything set within a content block will only affect the content within that block.

Another tweak could be to remove the duplication between the header and the title element's argument. Since they share the title, it would be convenient to store it in a place designed to hold metadata about the document. We would then need a way to retrieve the title in both places. The `document{:typst}` element can help us with the former: By using it in a set rule, we can store document metadata like title, description, and keywords.
...
Now, we need a way to retrieve the value we set in the main title and the header. Because the `title{:typst}` function is designed to work together with the `document{:typst}` element, calling it with no arguments will just print the title. For the header, we will need to be more explicit: Because Typst has no way of knowing that we want to insert the title there, we will need to tell it to do so manually.

Using _context,_ we can retrieve the contents of any values we have set on elements before. When we use the `context{:typst}` keyword, we can access any property of any element, including the document element's title property. Its use looks like this:
...
<TypstPreview
  ...
  ...
`}
  image='/typst-fumadocs/assets/15b893af691b49beed091b0e62baa2df.png'
  alt='Preview'
  editable={true}
/>
...
The paper above unfortunately looks like a wall of lead. To fix that, let's add some headings and switch our paper to a two-column layout. Fortunately, that's easy to do: We just need to amend our `page{:typst}` set rule with the `columns{:typst}` argument.

By adding `columns: 2{:typst}` to the argument list, we have wrapped the whole document in two columns. However, that would also affect the title and authors overview. To keep them spanning the whole page, we can wrap them in a function call to [`place{:typst}`](/docs/reference/layout/place/). Place expects an alignment and the content it should place as positional arguments. Using the named `scope{:typst}` argument, we can decide if the items should be placed relative to the current column or its parent (the page). There is one more thing to configure: If no other arguments are provided, `place{:typst}` takes its content out of the flow of the document and positions it over the other content without affecting the layout of other content in its container:
...
<TypstPreview
  ...
  #lorem(30)
`}
  image='/typst-fumadocs/assets/df4b3b714f57dfa956dbceab25713747.png'
  alt='Preview'
  editable={true}
/>

If we hadn't used `place{:typst}` here, the square would be in its own line, but here it overlaps the few lines of text following it. Likewise, that text acts as if there was no square. To change this behavior, we can pass the argument `float: true{:typst}` to ensure that the space taken up by the placed item at the top or bottom of the page is not occupied by any other content.
...
<TypstPreview
  ...
  #lorem(200)
`}
  image='/typst-fumadocs/assets/7423f42127d2f3c6b66ce2764863df00.png'
  alt='Preview'
  editable={true}
/>

In this example, we also used the `clearance{:typst}` argument of the `place{:typst}` function to provide the space between it and the body instead of using the [`v{:typst}`](/docs/reference/layout/v/) function. We can also remove the explicit `align(center, ..){:typst}` calls around the various parts since they inherit the center alignment from the placement.

Now there is only one thing left to do: Style our headings. We need to make them centered and use small capitals. These properties are not available on the `heading{:typst}` function, so we will need to write a few show-set rules and a show rule:

- A show-set rule to make headings center-aligned
- A show-set rule to make headings 13pt large and use the regular weight
- A show rule to wrap the headings in a call to the `smallcaps{:typst}` function
...
<TypstPreview
  ...
  ...
`}
  image='/typst-fumadocs/assets/484bf0b02bc25230ddf8cb08f231826c.png'
  alt='Preview'
  editable={true}
/>

This looks great! We used show rules that apply to all headings. In the final show rule, we applied the `smallcaps{:typst}` function to the complete heading. As we will see in the next example, we can also provide a custom rule to completely override the default look of headings.

The only remaining problem is that all headings look the same now. The "Motivation" and "Problem Statement" subsections ought to be italic run-in headers, but right now, they look indistinguishable from the section headings. We can fix that by using a `where{:typst}` selector on our show rule: This is a [method](/docs/reference/scripting/#methods) we can call on headings (and other elements) that allows us to filter them by their properties. We can use it to differentiate between section and subsection headings:
...
`}
  image='/typst-fumadocs/assets/c988e50827cad35a48eef259d8ef12ae.png'
  alt='Preview'
  editable={true}
/>

In this example, we first scope our previous rules to first-level headings by using `.where(level: 1){:typst}` to make the selector more specific. Then, we add a show-set rule for the second heading level. Finally, we need a show rule with a custom function: Headings enclose their contents with a block by default. This has the effect that the heading gets its own line. However, we want it to run into the text, so we need to provide our own show rule to get rid of this block.

We provide the rule with a function that takes the heading as a parameter. This parameter is conventionally called `it{:typst}`, but can have another name. The parameter can be used as content and will just display the whole default heading. Alternatively, when we want to build our own heading instead, we can use its fields like `body{:typst}`, `numbering{:typst}`, and `level{:typst}` to compose a custom look. Here, we are just printing the body of the heading with a trailing dot and leave out the block that the built-in show rule produces. Note that this heading will no longer react to set rules for heading numbering and similar because we did not explicitly use `it.numbering{:typst}` in the show rule. If you are writing show rules like this and want the document to remain customizable, you will need to take these fields into account.

This looks great! We wrote show rules that selectively apply to the first and second level headings. We used a `where{:typst}` selector to filter the headings by their level. We then rendered the subsection headings as run-ins. We also automatically add a period to the end of the subsection headings.
...
<img src="/typst-fumadocs/assets/3-advanced-paper" alt="The finished paper" style={{boxShadow: '0 4px 12px rgb(89 85 101 / 20%)', width: '500px', maxWidth: '100%', display: 'block', margin: '24px auto'}} />

## Review

You have now learned how to create titles, headers, and footers, how to use functions, show-set rules, and scopes to locally override styles, how to create more complex layouts with the [`grid{:typst}`](/docs/reference/layout/grid/) function, how to access element properties with context, and how to write show rules for individual functions, and the whole document. You also learned how to use the [`where{:typst}` selector](/docs/reference/styling/#show-rules) to filter the headings by their level.
