...
- Added support for citation collapsing (e.g. `[1]-[3]{:typst}` instead of `[1], [2], [3]{:typst}`) if requested by a CSL style
...
- Page ranges in `.bib{:typst}` files can now be arbitrary strings
- Multi-line values in `.bib{:typst}` files are now parsed correctly
- Entry keys in `.bib{:typst}` files now allow more characters
- Fixed error message for empty dates in `.bib{:typst}` files
- Added support for years of lengths other than 4 without leading zeros in `.bib{:typst}` files
- More LaTeX commands (e.g. for quotes) are now respected in `.bib{:typst}` files

## Visualization

- Added support for [patterns](/docs/reference/visualize/tiling/) as fills and strokes
- The `alpha{:typst}` parameter of the [`components{:typst}`](/docs/reference/visualize/color/#definitions-components) function on colors is now a named parameter **(Breaking change)**
...
- CJK text can now be emphasized with the `*{:typst}` and `_{:typst}` syntax even when there are no spaces
...
- Fixed [CJK-Latin-spacing](/docs/reference/text/text/#parameters-cjk-latin-spacing) before line breaks and in [`locate{:typst}`](/docs/reference/introspection/locate/) calls
- Fixed line breaking at the end of links

## Math

- Added [`mid{:typst}`](/docs/reference/math/lr/#functions-mid) function for scaling a delimiter up to the height of the surrounding [`lr{:typst}`](/docs/reference/math/lr/#functions-lr) group
- The [`op{:typst}`](/docs/reference/math/op/) function can now take any content, not just strings
- Improved documentation for [math alignment](/docs/reference/math/#alignment)
- Fixed swallowing of trailing comma when a symbol is used in a function-like way (e.g. `pi(a,b,){:typst}`)

## Scripting

- Any non-identifier dictionary key is now interpreted as an expression: For instance, `((key): value){:typst}` will create a dictionary with a dynamic key
- The [`stroke{:typst}`](/docs/reference/visualize/stroke/) type now has a constructor that converts a value to a stroke or creates one from its parts
- Added constructor for [`arguments{:typst}`](/docs/reference/foundations/arguments/) type
- Added [`calc.div-euclid{:typst}`](/docs/reference/foundations/calc/#functions-div-euclid) and [`calc.rem-euclid{:typst}`](/docs/reference/foundations/calc/#functions-rem-euclid) functions
- Fixed equality of [`arguments{:typst}`](/docs/reference/foundations/arguments/)
- Fixed [`repr{:typst}`](/docs/reference/foundations/repr/)of [`cmyk{:typst}`](/docs/reference/visualize/color/#definitions-cmyk) colors
...
- PDF compilation output is now again fully byte-by-byte reproducible if the document's [`date{:typst}`](/docs/reference/model/document/#parameters-date) is set manually
...
- Fixed a major bug where `typst watch{:typst}` would confuse files and fail to pick up updates
- Fetching of the release metadata in `typst update{:typst}` now respects proxies
- Fixed bug with `--open{:typst}` flag on Windows when the path contains a space
- The `TYPST_FONT_PATHS{:typst}` environment variable can now contain multiple paths (separated by `;{:typst}` on Windows and `:{:typst}` elsewhere)
...
- The [`number-align{:typst}`](/docs/reference/model/enum/#parameters-number-align) parameter on numbered lists now also accepts vertical alignments
- Fixed selectors on [quote](/docs/reference/model/quote/) elements
- Fixed parsing of `#return{:typst}` expression in markup
- Fixed bug where inline equations were displayed in equation outlines
- Fixed potential CRLF issue in [`raw{:typst}`](/docs/reference/text/raw/) blocks
- Fixed a bug where Chinese numbering couldn't exceed the number 255

## Development

- Merged `typst{:typst}` and `typst-library{:typst}` and extracted `typst-pdf{:typst}`, `typst-svg{:typst}`, and `typst-render{:typst}` into separate crates
- The Nix flake now includes the git revision when running `typst --version{:typst}`
