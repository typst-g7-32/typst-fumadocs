...
`}
  image='/typst-fumadocs/assets/1b3385d4711d911b023f85c4b77c69d8.png'
  alt='Preview'
  editable={true}
/>
...
<TypstPreview
  ...
  = Heading
`}
  image='/typst-fumadocs/assets/cea9024d2a17cd0d7d7ffffffc6673eb.png'
  alt='Preview'
  editable={true}
/>
...
- Use underscores / [`emph{:typst}`](/docs/reference/model/emph/) instead of the [`text{:typst}`](/docs/reference/text/text/) function to make text emphasized
- Use stars / [`strong{:typst}`](/docs/reference/model/strong/) instead of the text function to make text carry strong emphasis
- Use lists ([`list{:typst}`](/docs/reference/model/list/), [`enum{:typst}`](/docs/reference/model/enum/), [`terms{:typst}`](/docs/reference/model/terms/)) instead of normal text with newlines when working with itemized or ordered content
- Use [`quote{:typst}`](/docs/reference/model/quote/) for inline and block quotes
- Use the built-in [`bibliography{:typst}`](/docs/reference/model/bibliography/) and [`cite{:typst}`](/docs/reference/model/cite/) functions instead of manually printing a bibliography
- Use labels and [`ref{:typst}`](/docs/reference/model/ref/) or `@references{:typst}` to reference other parts of your documents instead of just typing out a reference
- Use the [`caption{:typst}` argument of the `figure{:typst}` element](/docs/reference/model/figure/#definitions-caption) to provide captions instead of adding them as text below the function call
...
`}
  image='/typst-fumadocs/assets/a870bd2924e6c08865ca0ef3b0046e7c.png'
  alt='Preview'
  editable={true}
/>

The show-set rule completely changes the default appearance of the [`strong{:typst}`](/docs/reference/model/strong/) element, but its semantic meaning will be preserved. If you need even more customization, you can provide show rules with fully custom layout code and Typst will still retain the semantic purpose of the element.
...
Typst automatically tags many layout artifacts such as headers, footers, page back- and foregrounds, and automatic hyphenation as artifacts. However, if you'd like to add purely decorative content to your document, you can use the [`pdf.artifact{:typst}`](/docs/reference/pdf/artifact/) function to mark a piece of content as an artifact. If you are unsure if you should mark an element as an artifact, ask yourself this: Would it be purely annoying if a screen reader announced the element to you? Then, it may be an artifact. If, instead, it could be useful to have it announced, then it is not an artifact.

For technical reasons, once in an artifact, content cannot become semantic again. To stack artifacts and semantic contents, use [`place{:typst}`](/docs/reference/layout/place/) to move the contents on top of one another.

Please note that Typst will mark shapes and paths like [`square{:typst}`](/docs/reference/visualize/square/) and [`circle{:typst}`](/docs/reference/visualize/circle/) as artifacts while their content will remain semantically relevant and accessible to AT. If your shapes have a semantic meaning, please wrap them in the [`figure{:typst}`](/docs/reference/model/figure/) element to provide an alternative textual description.
...
<div style={{display: 'flex', gap: '8px 16px', width: '100%', flexWrap: 'wrap', margin: '24px auto'}}>
 <img src="/typst-fumadocs/assets/chart-bad-regular.png" alt="Bar chart showing Energy production in Germany by kind in terawatt-hours on the X axis and the year on the y-axis. Each bar has up to four segments, for Nuclear (violet), Renewables (green), Fossil Fuels (red), and Other (blue). There is a legend in the top right corner associating the segment colors with their labels" style={{boxShadow: '0 4px 12px rgb(89 85 101 / 20%)', width: '200px', maxWidth: '100%', height: 'auto', display: 'block', borderRadius: '6px', flexGrow: '1'}} width="958" height="637" /> <img src="/assets/chart-bad-deuteranopia" alt="The same bar chart with changed colors, with the segments for Nuclear and Other in a very similar dark blue, and the neighboring segments of Renewables and Fossil Fuels in two almost indistinguishable shades of sickly yellow" style={{boxShadow: '0 4px 12px rgb(89 85 101 / 20%)', width: '200px', maxWidth: '100%', height: 'auto', display: 'block', borderRadius: '6px', flexGrow: '1'}} width="958" height="637" /> 
</div>
...
<div>
 <img src="/typst-fumadocs/assets/chart-good" alt="The same bar chart with the original colors. This time, black outlines around each segment are added. Additionally, each segment has a unique pattern." style={{boxShadow: '0 4px 12px rgb(89 85 101 / 20%)', width: '500px', maxWidth: '100%', height: 'auto', display: 'block', margin: '24px auto', borderRadius: '6px'}} width="958" height="637" /> 
</div>
...
<TypstPreview
  ...
  up to 2.3 m.
`}
  image='/typst-fumadocs/assets/c19e066f538edb97ce57f630f6cb56a8.png'
  alt='Preview'
  editable={false}
/>
...
Like the image function, the figure function has a [`alt{:typst}` attribute](/docs/reference/model/figure/#parameters-alt). When you use this attribute, many screen readers and other AT will not announce the content inside of the figure and instead just read the alternative description. Your alternative description must be comprehensive enough so that the AT user does not need to access the body of the figure. Only use the alternative description if the content of the figure are not otherwise accessible. For example, do not use the `alt{:typst}` attribute of a figure if it contains a `table{:typst}` element, but do use it if you used shapes within that come with a semantic meaning. If you specify both `alt{:typst}` and `caption{:typst}`, both will be read by AT. When your figure contains an image, set the alternative description on the [image itself](/docs/reference/visualize/image/#parameters-alt), not on the figure. Do not set both, as the image description would be overridden by the figure description.
...
- `zxx{:typst}` for text that is not in a natural language
- `und{:typst}` for text for which you cannot determine the natural language
- `mis{:typst}` for text in languages that have not been assigned a language code
...
`}
  image='/typst-fumadocs/assets/e96165d4834d11194fdaf6bee05bd86b.png'
  alt='Preview'
  editable={true}
/>
...
Most likely, you will also want to include the title visibly in your document. To do so, use the [`title{:typst}`](/docs/reference/model/title/) element. When you add a call to the title element without any arguments, it will print the contents of what you set as the document's title. Alternatively, you can customize the title by passing content as the positional body argument. Do not use the title element more than once in your document.

Never use a heading for your document title; instead, use the title element. Should you have experience with HTML, it is important to remember that the semantics of the heading element in Typst differ from HTML headings. It is encouraged to use multiple first-level headings for section headings in Typst documents. When exporting to HTML, a [title](/docs/reference/model/title/) will be serialized as a `h1{:typst}` tag while a [first-level heading](/docs/reference/model/heading/#parameters-level) will be serialized as a `h2{:typst}` tag. In PDF export, the title and headings will be correctly tagged based on the PDF version targeted.
